...
export default function SubmitEventPage() {
  ...
  const tAdmin = useTranslations("adminNew");
  ...
  const handleAnalyze = async (
    ...
    try {
      ...
      setForm((prev) => ({
        ...
        event_url: data.external_link ?? prev.event_url,
      }));
      setMissingFields(
        Object.keys(data).filter((k) => data[k] === null)
      );
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      if (!error && data?.id) {
        ...
        await supabase.functions.invoke("send-email-on-new-event", {
  ...
  return (
    <div className="p-4 max-w-xl mx-auto">
      ...
      {analysisLoading && (
        <p className="text-sm">{tAdmin("analyzing")}</p>
      )}
      {analysisError && (
        <p className="text-sm text-red-600">{tAdmin("analysisError")}</p>
      )}
      {screenshotPreview && (
        <img src={screenshotPreview} alt="preview" className="mb-4 max-h-48" />
      )}
      {missingFields.length > 0 && (
        <p className="text-sm text-yellow-600">
          {tAdmin("missing")} {missingFields.join(", ")}
        </p>
      )}
      <button
        type="button"
